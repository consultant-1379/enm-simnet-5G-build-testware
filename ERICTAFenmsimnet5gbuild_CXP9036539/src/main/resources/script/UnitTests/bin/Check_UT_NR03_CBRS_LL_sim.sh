...
if [[ $NE == *"00003" ]];then cellcount=3; else cellcount=2; fi
   if [[ $cellsize -ne  $cellcount ]]
   then
	echo -e "\033[0;31mFAILED\033[m:Node $NE doesn't have $cellsize cells " | tee  -a ut_result_$SIM.txt;return 1
   	...
   	do
		...
		#echo -e 'e csmodb:get_mo_attribute_by_id(null,'$DU',cbrsCell).' | ~/inst/netsim_shell -sim $SIM -ne $NE > cbrs.txt
		#cbrsCell=$(cat cbrs.txt | tail -1 | cut -d "," -f 3)
		#echo -e cbrscell : $cbrsCell

		if [ $admin_state -ne 1 ];then echo -e "\033[0;31mFAILED\033[m: $NE - adminsistrtaiveState value is not UNLOCKED"  | tee -a ut_result_$SIM.txt;return 1; fi
...
function check_sectorCarrier_config(){
  ...
  if [[ $NE == *"00003" ]];then sccount=3; else sccount=2; fi
  if [[ $scsize -ne $sccount ]]
  then
	echo -e "\033[0;31mFAILED\033[m:Node $NE doesn't have $sccount SectorCarrier mos" | tee -a ut_result_$SIM.txt;return 1 
...
#SIM="NR21-Q4-V1x5-gNodeBRadio-NRAT-NR03"
SIM=$1
	if [[ $SIM != "NR21-Q4-V1x5-gNodeBRadio-NRAT-NR03" ]];then
	...
	NodesList=("NR03gNodeBRadio00002" "NR03gNodeBRadio00003")
	for NE in ${NodesList[@]}
	do
